...
async function load() {
  // GitHub Pages supports fetch(), but use the embedded content first so the
  // portfolio never appears empty while content.json is loading.
  data = window.PORTFOLIO_CONTENT || null;
  if (data) render();
  ...
  try {
    ...
    console.warn('content.json could not be fetched; using embedded content.', e);
  }

  if (!data) {
    const heroBio = $('#heroBio');
    if (heroBio) heroBio.textContent = 'Konten portfolio belum dapat dimuat.';
  }
}

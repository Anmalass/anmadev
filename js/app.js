...
function render() {
  ...
  if (data.anime.video) {
    ...
    if (/youtube\.com|youtu\.be/.test(u)) {
      ...
      $('#animeVideo').innerHTML = `<iframe src="https://www.youtube.com/embed/${encodeURIComponent(id)}" title="Anime motion" allowfullscreen></iframe>`;
    } else $('#animeVideo').innerHTML = `<video controls playsinline src="${esc(u)}"></video>`;
  }
